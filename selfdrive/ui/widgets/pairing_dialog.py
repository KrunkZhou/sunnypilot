...
class PairingDialog(Widget):
  ...
  def _get_pairing_url(self) -> str:
    try:
      ...
      token = ""
    return f"https://connect.krunk.cn/?pair={token}"
  ...
  def _render(self, rect: rl.Rectangle) -> int:
    ...
    # Title
    title = tr("Pair your device to connect.krunk.cn")
  ...
  def _render_instructions(self, rect: rl.Rectangle) -> None:
    instructions = [
      tr("Go to https://connect.krunk.cn on your phone"),
      tr("Click \"add new device\" and scan the QR code on the right"),
      tr("Bookmark connect.krunk.cn to your home screen to use it like an app"),
    ]
