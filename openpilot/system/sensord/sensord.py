...
def wait_for_gpio(event: threading.Event) -> int | None:
  # Offroad Sentry can start sensord before udev has created the GPIO device
  # and granted access. Keep this worker alive without changing permissions.
  retry_delay = 0.1
  while not event.is_set():
    try:
      return gpiochip_get_ro_value_fd("sensord", 0, 84)
    except (PermissionError, FileNotFoundError) as exc:
      cloudlog.warning(f"Sensor interrupt GPIO is not ready: {exc}; retrying in {retry_delay:.1f}s")
      if event.wait(retry_delay):
        break
      retry_delay = min(retry_delay * 2, 5.0)
  return None


def interrupt_loop(sensors: list[tuple[Sensor, str, bool]], event: threading.Event) -> None:
