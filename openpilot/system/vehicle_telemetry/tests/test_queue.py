...
class TestQueue(unittest.TestCase):
  def setUp(self):
    ...
    self.db = Outbox(self.root / "outbox.db", BOOT)
    self.db.set_meta("epoch", 1_800_000_000_000)
  ...
  def test_corrected_queue_survives_reboot_and_claim_recovers(self):
    payload = self.make_payload()
    self.db.save_latest(payload["snapshot"])
    ordering = (self.db.meta("epoch"), self.db.meta("seq"))
    _, rows = self.db.claim(NANO)
    self.assertEqual(len(rows), 1)
    self.db.close()
    self.db = Outbox(self.root / "outbox.db", str(uuid.uuid4()))
    self.assertEqual((self.db.meta("epoch"), self.db.meta("seq")), ordering)
    self.assertEqual(self.db.meta("latest"), payload["snapshot"])
  ...
  def test_persisted_highwater_is_incremented_without_wall_time(self):
    self.assertEqual(self.db.meta("epoch"), 1_800_000_000_001)

  def test_fresh_database_ordering_survives_reopen(self):
    path = self.root / "fresh.db"
    fresh = Outbox(path, BOOT)
    try:
      self.assertEqual(fresh.new_epoch(), 1)
      self.assertEqual(fresh.next_order(), (1, 1))
    finally:
      fresh.close()
    reopened = Outbox(path, str(uuid.uuid4()))
    try:
      self.assertEqual(reopened.new_epoch(), 2)
      self.assertEqual(reopened.next_order(), (2, 1))
    finally:
      reopened.close()
