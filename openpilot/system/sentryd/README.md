...
Schema 4 uses thresholds `0.02` (High), `0.04` (Standard), or `0.08` (Low) m/s². Confirmation is fixed at **10 qualifying motion samples**; the compatibility file `warning_persistence_seconds` must contain `1` and is no longer adjustable. `wait_for_driver_exit` is `0` or `1` and defaults to `1` (on). `infer_lock_from_flashes` is `0` or `1` and defaults to `0` (off). Other defaults remain disabled, consent 0, and `0.04` m/s². Effective enablement requires `enabled=1` and the current consent version. First enablement commits consent before enablement. A one-time legacy import preserves sensitivity, normalizes valid warning timing to `1`, forces disabled/no-consent, durably commits the folder, and then removes safe regular legacy Sentry keys.

A complete valid schema-1, schema-2, or schema-3 folder is upgraded under the exclusive configuration lock. Schema 1 gains the default-on driver-exit file; schema 1/2 warning values `0.5`, `1`, `2`, and `5` normalize to `1` (schema 3 already requires `1`). The new default-off flash setting and all migrated fields are durable before the schema marker changes to `4`. Valid staged files from interrupted migration are preserved and re-synced; corrupt, incomplete, or future-version data is not overwritten. Migration preserves enablement, consent, sensitivity, the driver-exit setting, the outbox, and media. Older binaries cannot read the upgraded configuration.
...
### Optional flash-based lock control

The MICI **lock/unlock flash detection** toggle is off by default. This experimental Audi A3 Mk3/classic VW MQB feature infers lock state from indicator acknowledgement flashes; it does not read or prove actual central-lock status. Before enabling it on a car, verify that parked acknowledgements reach the Comma-visible bus, match the bounds below, and remain distinguishable from active hazards. Unsupported/missing indicator data produces an actionable status and never invents a lock/unlock action or clears an existing pause.

The existing passive CAN reader also decodes `Blinkmodi_02` (`0x366`, eight bytes): actual exterior left/right bulb bits, `Hazard_Switch` (four-way flashers active), and the 1 Hz idle / 50 Hz active rate flag. Comfort/stalk indicators and the hazard-button signal are not pulse counters. It never transmits CAN. A complete observed single synchronized pulse means inferred **locked**, two mean inferred **unlocked**. Hazards, one-sided/third pulses, initial ON, malformed/reordered/stale data, and incomplete bursts are rejected. A rejected sequence requires two seconds of observed clean OFF before another burst can qualify, preventing recognition of only its final pulse.

Fixed bounds are 0.10–1.20 seconds per pulse, 0.10–1.50 seconds between pulses, two seconds of observed OFF to finish, and six seconds maximum per burst. Sample gaps may be at most 0.25 seconds at the prior 50 Hz rate or 1.50 seconds at the prior 1 Hz rate. Silence is never evidence of OFF and cannot complete a burst. Recognition precedes motion confirmation and ordinary door handling; while classifying, new automatic captures and confirmation wait, but existing provisional work still expires or is discarded if its sensors become invalid/stale. Door-opening handling is deferred until classification completes.

Unlock suppresses **all new automatic motion and door captures** and offroad sensor demand for 60 minutes. It discards unconfirmed motion, but preserves earlier confirmed queued/in-progress work, uploads, and explicit manual tests. Another unlock restarts the hour. After the hour expires, all doors and the trunk must be observed closed before a fresh 90-second arming timer starts. Reopening during this post-unlock arming resets the timer until everything closes.

A validated one-flash lock cancels the unlock pause and the driver-exit/five-minute door wait, then starts **90-second arming immediately without checking door closure**. Open or unknown doors, later door openings, and loss of the door receiver do not delay or reset this lock-triggered timer. Revision-21 durable-finalization/rearming requirements always remain in force for either path.

The unlock/pending-rearm marker is committed before canceling provisional motion. Daemon or device restart during an unlock pause starts a **new full 60-minute monotonic wait**. Restart during lock-triggered arming starts a fresh 90 seconds without requiring door data. Restart during post-unlock arming requires a fresh all-closed observation and a new 90 seconds. On that post-unlock path, normal CAN sleep does not erase an observed closure; restart or receiver/profile loss does. Positive ignition clears lock control; unknown ignition or configuration-read failures do not erase a saved pause. Explicitly turning the toggle off clears its marker and returns to the existing Sentry gates. Persistent Sentry enablement is never changed by a flash inference. This feature adds no RTZ event, wire schema, or webhook.
