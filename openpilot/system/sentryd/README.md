...
Outbox schema 3 durably records server-directed retry deadlines separately from local backoff. Upgrade is atomic and preserves queued request bytes, media, attempt history, and active claims. Older outboxes did not distinguish these waits, so their existing HTTP-failure deadlines are conservatively retained during automatic recovery; explicit retry-all can clear them. This is an internal database migration, not a change to folder configuration or the RTZ event-wire schema. Older device binaries supporting only outbox schema 2 cannot read the upgraded queue.
