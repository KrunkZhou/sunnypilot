...
BUNDLED_SPEED_LIMIT_DATABASE_PATH = Path(BASEDIR) / "openpilot" / "sunnypilot" / "mapd" / "data" / SPEED_LIMIT_DATABASE_NAME
...
def decode_geometry(blob: bytes) -> list[tuple[float, float]]:
  ...
  if any(not (-180.0 <= longitude <= 180.0 and -90.0 <= latitude <= 90.0) for longitude, latitude in points):
    raise ValueError("invalid speed-limit geometry coordinates")
  return points
...
class SpeedLimitDatabase:
  ...
  def _connect(self) -> sqlite3.Connection | None:
    ...
    try:
      ...
      if schema_version is None or int(schema_version[0]) != 1:
        raise ValueError("unsupported speed-limit database schema")
  ...
  def lookup(self, position: Position | None, bearing: float | None) -> float:
    if position is None or bearing is None:
      return 0.0
    try:
      if not all(math.isfinite(value) for value in (position.latitude, position.longitude, bearing)):
        return 0.0
      if not (-90.0 <= position.latitude <= 90.0 and -180.0 <= position.longitude <= 180.0):
        return 0.0
      ...
      try:
        for speed_kph, direction, geometry in rows:
          try:
            ...
            if not math.isfinite(speed_kph) or speed_kph <= 0.0:
              continue
            distance, geometry_heading = closest_segment(position, decode_geometry(geometry))
          except (TypeError, ValueError, OverflowError, struct.error, zlib.error):
            continue
